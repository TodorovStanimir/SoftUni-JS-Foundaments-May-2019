comment = (input) => {
    let listUsers = [];
    let listArticles = new Map();
    input.forEach(line => {
        if (line.startsWith('user ')) {
            let userName = line.split('user ').filter(el => el !== '')[0];
            listUsers.push(userName);
        } else if (line.startsWith('article ')) {
            let articleName = line.split('article ').filter(el => el !== '')[0];
            listArticles.set(articleName, []);
        } else {
            ...
            if (listUsers.includes(userName) && listArticles.has(articleName)) {
                listArticles.get(articleName).push({ [userName]: { [commentTitle]: commentContence } })
            }
        }
    });
   [...listArticles]
